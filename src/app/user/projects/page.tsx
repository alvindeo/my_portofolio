...
export function ProjectsSection() {
  ...
  useEffect(() => {
    fetch('/api/projects')
      .then((r) => r.json())
      .then((d) => { if (Array.isArray(d) && d.length > 0) setDbProjects(d) })
      .catch(console.error)
  }, [])

  const featured = dbProjects.length > 0
    ? dbProjects.filter((p) => p.featured).slice(0, 3).map((p) => ({
        id: p.id, title: p.title, description: p.description,
        image: p.imageUrl ?? '', tags: p.techStack,
        liveUrl: p.liveUrl, githubUrl: p.githubUrl, featured: p.featured,
      }))
    : projectsData.filter((p) => p.featured).slice(0, 3).map((p) => ({
        id: p.id, title: p.title, description: p.description,
        ...
        {featured.map((project, i) => (
          ...
          >
            <div className="relative w-full h-56 overflow-hidden" style={{ background: 'var(--bg-secondary)' }}>
              <motion.img src={project.image} alt={project.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" onError={(e) => { (e.target as HTMLImageElement).style.visibility = 'hidden' }} />
            ...
            <div className="p-6 flex flex-col flex-1 gap-4">
              <div className="flex flex-wrap gap-2">{(project.tags as (string | {name:string;icon:string})[]).map((tag, j) => (<span key={j} className="tag">{typeof tag === 'string' ? tag : tag.name}</span>))}</div>
              ...
              <div className="flex items-center gap-2 pt-4 border-t" style={{ borderColor: 'var(--card-border)' }}>
                <Link href="/user/projects" style={{ color: 'var(--accent)' }} className="text-sm font-semibold">View Detail →</Link>
              </div>
            </div>
          </motion.div>
        ))}
...
export default function PublicProjectsPage() {
  ...
  useEffect(() => {
    fetch('/api/projects').then(r => r.json()).then(d => { if (Array.isArray(d)) setDbProjects(d) })
  }, [])

  const displayData = dbProjects.length > 0 ? dbProjects : projectsData
  
  const filtered = displayData.filter((p) => {
    const title = p.title || ''
    const desc = p.description || ''
    // normalize: techStack bisa TechItem[] atau string[]
    const rawTags: (string | {name:string;icon?:string})[] = (p as any).techStack || (p as any).tags || []
    const tagStrings: string[] = rawTags.map((t) => typeof t === 'string' ? t : t.name)

    const matchSearch = title.toLowerCase().includes(search.toLowerCase()) || desc.toLowerCase().includes(search.toLowerCase())
    const matchTag = activeTag === 'All' || tagStrings.includes(activeTag)
          ...
          {filtered.map((project, i) => (
            <motion.div key={project.id} custom={i} variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} className="project-card rounded-2xl overflow-hidden flex flex-col" style={{ background: 'var(--bg-card)', border: '1px solid var(--card-border)' }}>
              <div className="relative h-48 overflow-hidden" style={{ background: 'var(--bg-secondary)' }}>
                <img src={project.imageUrl || project.image} alt={project.title} className="w-full h-full object-cover" />
              </div>
              <div className="p-6 flex flex-col flex-1 gap-4">
                <div className="flex flex-wrap gap-2">{(project.techStack || project.tags || []).map((tag: string | {name:string;icon:string}, j: number) => (<span key={j} className="tag">{typeof tag === 'string' ? tag : tag.name}</span>))}</div>
                <h3 className="text-lg font-bold" style={{ fontFamily: "'Syne', sans-serif", color: 'var(--text-heading)' }}>{project.title}</h3>
