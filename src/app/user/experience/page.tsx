...
const experiences: Experience[] = [
  ...
  },
  {
    ...
    {
    role: "Active Member / Ministry Division",
    org: "Persekutuan Mahasiswa Kristen (PMK)",
    desc: "Actively participated in spiritual development programs and organizational activities, contributing to community building and character development within the Christian student fellowship.",
    year: "2024 - 2025",
    tags: ["Leadership", "Community Engagement", "Event Coordination", "Public Speaking", "Team Collaboration"],
    achievements: [
      "Contributed to the organization of fellowship events, seminars, and spiritual gatherings.",
      "Supported coordination of weekly services and special events with structured planning.",
      "Collaborated with internal committees to strengthen member engagement and participation.",
      "Assisted in documentation and communication to improve program effectiveness."
    ]
  }
];
...
export function ExperienceSection() {
  const [dbExp, setDbExp] = useState<DbExperience[]>([]);

  useEffect(() => {
    fetch('/api/experience').then((r) => r.json()).then((d) => { if (Array.isArray(d) && d.length > 0) setDbExp(d) })
  }, [])

  // Map DB data to card format, fallback to hardcoded
  const displayExp = dbExp.length > 0
    ? dbExp.slice(0, 2).map((e) => ({
        role: e.role,
        org: e.company,
        desc: e.description,
        year: e.period,
        tags: parseLines(e.tags),
      }))
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="relative">
          ...
          <div className="space-y-24">
            {displayExp.map((exp, i) => {  // ← data dari DB atau fallback
              const isEven = i % 2 === 0;
              return (
...
/**
 * ACCORDION CARD - Mobile collapsible experience item
 */
    ...
    >
      ...
      >
        <div className="flex-1 min-w-0">
          <p className="text-base font-bold" style={{ fontFamily: "'Syne', sans-serif", color: "var(--text-heading)" }}>
            {exp.role}
          </p>
...
export default function PublicExperiencePage() {
  ...
  useEffect(() => {
    fetch('/api/experience')
      .then((r) => r.json())
      .then((d) => { if (Array.isArray(d) && d.length > 0) setDbExp(d) })
      .catch(console.error)
  }, [])

  // Map DB or fallback to Experience[]
  const displayAll: Experience[] = dbExp.length > 0
    ? dbExp.map((e) => ({
  ...
  return (
    <div className="relative overflow-hidden" style={{ background: "var(--bg-primary)", color: "var(--text-primary)", minHeight: "100vh", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <main className="pt-32 pb-24 px-6 max-w-6xl mx-auto">
        ...
        {/* Detailed list: accordion on mobile, full cards on desktop */}
        <div className="space-y-4 md:space-y-12">
          {/* Mobile: accordion */}
          <div className="md:hidden space-y-3">
            {displayAll.map((exp: Experience, i: number) => (
              <AccordionExperienceCard key={i} exp={exp} index={i} />
            ))}
          </div>
